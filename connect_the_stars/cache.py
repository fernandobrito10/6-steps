...
def get_actor_by_name(query: str) -> Actor | None:
    conn = _get_conn()
    with _lock:
        row = conn.execute(
            """
...
def set_actor_by_name(query: str, actor: Actor | None) -> None:
    conn = _get_conn()
    now = _now_iso()
    with _lock:
...
def get_actor_search(query: str) -> list[Actor] | None:
    conn = _get_conn()
    with _lock:
        row = conn.execute(
...
def set_actor_search(query: str, actors: list[Actor]) -> None:
    conn = _get_conn()
    now = _now_iso()
    with _lock:
        for a in actors:
...
def get_actor_movies(actor_id: int) -> list[Movie] | None:
    conn = _get_conn()
    with _lock:
...
def set_actor_movies(actor_id: int, movies: list[Movie]) -> None:
    conn = _get_conn()
    now = _now_iso()
    with _lock:
        conn.execute("BEGIN")
...
def get_movie_cast(movie_id: int) -> list[tuple[Actor, int]] | None:
    conn = _get_conn()
    with _lock:
        rows = conn.execute(
            """
...
def set_movie_cast(movie_id: int, cast: list[tuple[Actor, int]]) -> None:
    conn = _get_conn()
    now = _now_iso()
    with _lock:
        conn.execute("BEGIN")
...
def get_movie_pt_title(movie_id: int) -> str | None:
    conn = _get_conn()
    with _lock:
        row = conn.execute(
...
def set_movie_pt_title(movie_id: int, title_pt: str | None) -> None:
    conn = _get_conn()
    with _lock:
        conn.execute(
            """
